...
namespace ctpg
{
    ...
    namespace utils
    {
        ...
        public:
            ...
            constexpr const char *name(char c) const
            {
                const size_t idx = char_to_idx(c);
                return arr[char_to_idx(c)];
            }
    ...
    {
    public:
        constexpr term(int precedence = 0, associativity a = associativity::no_assoc) : precedence(precedence), ass(a)
        {
        }
    ...
    public:
        ...
        constexpr char_term(char c, int precedence = 0, associativity a = associativity::no_assoc) : term(precedence, a), c(c)
        {
    ...
    public:
        ...
        constexpr string_term(const char (&str)[DataSize], int precedence = 0, associativity a = associativity::no_assoc) : term(precedence, a)
        {
    ...
    public:
        ...
        constexpr custom_term(const char *custom_name, Ftor ftor, int precedence = 0, associativity a = associativity::no_assoc) : term(precedence, a), custom_name(custom_name), ftor(ftor)
        {
        }
    ...
    struct parse_options
    {
        ...
        constexpr parse_options &set_skip_newline(bool val = true)
        {
            skip_newline = val;
            return *this;
        }
    ...
    struct recognized_term
    {
        ...
        constexpr recognized_term(size16_t term_idx, size_t len) : term_idx(term_idx), len(len)
        {
        }
    ...
    namespace detail
    {
        ...
        public:
            ...
            constexpr rule(L l, std::tuple<R...> r) : f(nullptr), l(l), r(r), precedence(0)
            {
            }

            template <typename F1>
            constexpr rule(F1 &&f, L l, std::tuple<R...> r) : f(std::move(f)), l(l), r(r), precedence(0)
            {
            }

            template <typename F1>
            constexpr rule(F1 &&f, L l, std::tuple<R...> r, int precedence) : f(std::move(f)), l(l), r(r), precedence(precedence)
            {
            }
    ...
    namespace detail
    {
        ...
        struct parse_state
        {
            constexpr parse_state(
                ...
                Iterator buffer_end,
                const ValueReductors &reductors) : cursor_stack(cursor_stack),
    ...
    public:
        constexpr parser(
            ...
            LexerUsage,
            Limits) : parser(grammar_root, terms, nterms, std::move(rules))
        {
        }
        ...
        constexpr parser(
            ...
            nterm_tuple_type nterms,
            rule_tuple_type &&rules,
            LexerUsage) : parser(grammar_root, terms, nterms, std::move(rules))
        {
        }
        ...
        constexpr parser(
            ...
            term_tuple_type terms,
            nterm_tuple_type nterms,
            rule_tuple_type &&rules) : term_tuple(terms),
                                       nterm_tuple(nterms),
    ...
    private:
        ...
        enum class parse_table_entry_kind : size8_t
        {
            error,
            success,
            shift,
        ...
        {
            constexpr state_analyzer(const grammar_info &gi, simple_state_table &simple_states, lr1_parse_table &parse_table) : gi(gi), simple_states(simple_states), parse_table(parse_table)
            {
            }
        ...
        constexpr void analyze_rules(std::index_sequence<I...>, const root_nterm_type &root)
        {
            ...
            stdex::sort(gi.rule_infos, [](const auto &ri1, const auto &ri2)
    ...
    namespace regex
    {
        ...
        namespace regex_parser
        {
            ...
            constexpr custom_term regex_digit_09("regex_digit_09", [](auto sv)
                                                 { return size32_t(sv[0]) - '0'; });
            ...
            constexpr parser regex_parser_object(
                ...
                rules(
                    ...
                    number(number, regex_digit_09) >= [](size32_t n, size32_t x)
                        { return n * 10 + x; },
                    primary(regex_digit_09) >>= [](auto &ctx, size32_t number)
                    { return ctx.primary_char(char(number + '0')); },
                    primary(regex_primary) >>= [](auto &ctx, const auto &s)
                    { return ctx.primary_subset(s); },
                    primary('(', expr, ')') >= _e2,
                    q_expr(primary),
                    q_expr(primary, '*') >>= [](auto &ctx, slice s, skip)
                    { return ctx.star(s); },
                    q_expr(primary, '+') >>= [](auto &ctx, slice s, skip)
                    { return ctx.plus(s); },
                    q_expr(primary, '?') >>= [](auto &ctx, slice s, skip)
                    { return ctx.opt(s); },
                    q_expr(primary, '{', number, '}') >>= [](auto &ctx, slice s, skip, size32_t n, skip)
                    { return ctx.rep(s, n); },
                    concat(q_expr),
                    concat(concat, q_expr) >>= [](auto &ctx, slice s1, slice s2)
                    { return ctx.cat(s1, s2); },
                    alt(concat),
                    alt(alt, '|', alt) >>= [](auto &ctx, slice s1, skip, slice s2)
                    { return ctx.alt(s1, s2); },
                    expr(alt)),
    ...
    public:
        ...
        constexpr regex_term(associativity a = associativity::no_assoc) : regex_term(nullptr, 0, a)
        {
        }

        constexpr regex_term(int precedence = 0, associativity a = associativity::no_assoc) : regex_term(nullptr, precedence, a)
        {
        }

        constexpr regex_term(const char *custom_name, int precedence = 0, associativity a = associativity::no_assoc) : term(precedence, a),
